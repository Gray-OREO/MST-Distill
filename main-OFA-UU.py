...
if __name__ == '__main__':
    ...
    '''
    parser = ArgumentParser(description='CML-OFA-UU')
    ...
    parser.add_argument('--Tmodel', type=str, default='MLP-Tb',
    ...
    parser.add_argument('--ckpt_name', type=str,
                        default='CMMD-V2_MLP-Tb_seed0_m2_ep92-100.pth',
    ...
    parser.add_argument('--commit', type=str, default='CMKD-OFA-UU',
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for i, (data, data2, label) in tqdm(enumerate(valid_loader), desc="Model Validating ...",
                ...
                loss = loss_s
                L_s_val += loss.item()
                L_ofa_val += loss_ofa.item()
                metric.update(outputs_s, label)
            L_s_val = L_s_val / (i + 1)
            L_ofa_val = L_ofa_val / (i + 1)
        # res = metric.compute()
        ...
        # For NYU-Depth-V2
        print(f"Valid =====> Epoch {epoch + 1}/{args.epochs}: loss = {L_s_val:.4f} | loss_ofa = {L_ofa_val:.4f}")
        writer.add_scalar('valid/Loss', L_s_val, epoch)
