...
if __name__ == '__main__':
    ...
    '''
    parser = ArgumentParser(description='CML-KD')
    ...
    parser.add_argument('--ckpt_name', type=str,
                        default='AV-MNIST_ThreeLayerCNN-A_seed0_m2_ep24-100.pth',
    ...
    parser.add_argument('--commit', type=str, default='UUKD',
    ...
    model_t.load_state_dict(torch.load(f'checkpoints/stu/wo_kd/{args.ckpt_name}', weights_only=True, map_location=f'cuda:{args.cuda_id}'))
    model_t.eval()
    ...
    for epoch in range(args.epochs):
        ...
        print(f'Train =====> Epoch {epoch + 1}/{args.epochs}: loss = {tra_LOSS_s_avg:.4f} | '
              f'loss_task = {tra_LOSS_task_avg:.4f} | loss_kl = {tra_LOSS_kl_avg:.4f}')
        ...
        # For NYU-Depth-V2
        print(f"Valid =====> Epoch {epoch + 1}/{args.epochs}: loss = {L_s_val:.4f} | loss_task = {Loss_s_avg:.4f} | loss_kl = {Loss_kl_avg:.4f}")
        writer.add_scalar('valid/Loss', L_s_val, epoch)
    ...
    if args.save_model:
        if not os.path.exists('./checkpoints/stu/kd'):
            os.makedirs('./checkpoints/stu/kd')
        names = args.ckpt_name.split('_')
        Tmodel_mode = names[3]
        torch.save(best_model_state,
                   f'./checkpoints/stu/kd/{args.database}_{args.Tmodel}_{Tmodel_mode}--{args.Smodel}_seed{args.seed}_{args.mode}_ep{best_epoch + 1}-{args.epochs}.pth')
